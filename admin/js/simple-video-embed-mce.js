				...
				onclick : function() {
					...
					var win = editor.windowManager.open({
						...
						onsubmit : function(e) {
							...
							url += (e.data.type !== "video") ? "playlistItems?playlistId=" + id : "videos?id=" + id;
							url += "&key=" + editor.getLang('wpsve_mce_button.googlePublicKeyV3') + "&part=id";
							...
							$.ajax({
								...
								error : function(xhr, status, err) {
									...
									switch (xhr.status) {
									case 0:
										editor.windowManager.alert(xhr.statusText);
										break;
									case 400:
										if (xhr.responseJSON) {
											switch (xhr.responseJSON.error.errors[0].reason) {
											case "keyInvalid":
												editor.windowManager.alert("Error status " + xhr.status + ": " + editor.getLang('wpsve_mce_button.alertKeyInvalid'));
												break;
											case "required":
												editor.windowManager.alert("Error status " + xhr.status + ": " + editor.getLang('wpsve_mce_button.alertRequiredSend'));
												break;
											case "missingRequiredParameter":
												editor.windowManager.alert("Error status " + xhr.status + ": " + editor.getLang('wpsve_mce_button.alertRequiredSend'));
												break;
											case "unknownPart":
												editor.windowManager.alert("Error status " + xhr.status + ": " + editor.getLang('wpsve_mce_button.alertUnknownPart'));
												break;
											default:
												editor.windowManager.alert("Error status " + xhr.status + ": " + xhr.statusText);
											}
										} else {
											editor.windowManager.alert("Error status " + xhr.status + ": " + xhr.statusText);
										}
									...
									case 403:
										if (xhr.responseJSON) {
											editor.windowManager.alert("Error status " + xhr.status + ": " + xhr.responseJSON.error.message);
										} else {
											editor.windowManager.alert("Error status " + xhr.status + ": " + xhr.statusText);
										}
										break;
									case 404:
										editor.windowManager.alert("Error status " + xhr.status + ": " + editor.getLang('wpsve_mce_button.alertUrlNotFound') + " " + url);
										break;
									default:
										editor.windowManager.alert("Error status " + xhr.status + ": " + xhr.statusText);
									}
								...
								if (data.items.length == 0) {
									error = true;
									editor.windowManager.alert("Error: " + editor.getLang('wpsve_mce_button.' + e.data.type + 'Label') + " " + editor.getLang('wpsve_mce_button.alertNotFound'));
								}
